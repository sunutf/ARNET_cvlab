...
class TSN_Ada(nn.Module):
    ...
    def backbone(self, input_data, the_base_model, r_result,  new_fc, signal=-1, indices_list=[], boost=False, b_t_c=False,
                 **kwargs):
        #r_all : B, T, :reso_dimo + skip_dim
        ...
        if b_t_c:
            ...
            if r_result is not None:
                ...
                for t_i in range(_t):
                    if r_result[0,t_i] == 1:
                        feat_t.append(the_base_model(input_2d[t_i].unsqueeze(0)))
            
                if len(feat_t) != 0:
                    feat = torch.stack(feat_t, dim=0)
                else:
                    return None, None
            else:
            ...
            if new_fc is not None:
                ...
                if r_result is not None:
                    ...
                    for i in range(len(feat_t)):
                        base_out_t.append(new_fc(feat_t[i].unsqueeze(0)))
                    _base_out = torch.stack(base_out_t, dim=0)
                else:
    ...
    def get_lite_j_and_r(self, input_list, online_policy, tau):
        ...
        feat_lite, _ = self.backbone(input_list[self.args.policy_input_offset], self.lite_backbone, None, None)
    ...
    def get_feat_and_pred(self, input_list, r_all, **kwargs):
        ...
        for bb_i, the_backbone in enumerate(self.base_model_list):
            if (self.args.backbone_list[0] == self.args.backbone_list[1]):
                ...
                feat_out, base_out = self.backbone(input_list[bb_i], the_backbone, r_all[:,:,bb_i], self.new_fc_list[bb_i])
    ...
    def forward(self, *argv, **kwargs):
        if not self.args.ada_reso_skip:  # TODO simple TSN
            total_base_out = torch.zeros([1, 1, self.num_class]).cuda()
            _b,_tc,_h,_w = kwargs["input"][0].shape
            _t, _c = _tc // 3 ,3
            _input = kwargs["input"][0].view(_b*_t,_c,_h,_w)
            total_cnt = self.time_steps
            for t_i in range(self.time_steps):
                _, base = self.backbone(_input[t_i].unsqueeze(0), self.base_model, None, self.new_fc,
                                        signal=self.args.default_signal)
                total_base_out += base
                if self.early_termination(total_base_out/float(t_i+1), t_i):
                    total_cnt = t_i+1
                    break
            ...
            output = total_base_out/float(total_cnt)
            #output = self.consensus(base_out)
            return output.squeeze(1), total_cnt
        ...
        if self.args.use_reinforce:
            ...
            r_all = torch.zeros([batch_size, 16, 7]).cuda()
            r_all[:,:,0] = 1
#             output = torch.randn(1,1,self.num_class).cuda()
#             return output.squeeze(1), r_all, None, None, None
        ...
        if self.args.offline_lstm_last:  # TODO(yue) no policy - use policy net as backbone - just LSTM(last)
            ...
            if self.args.save_meta and self.args.save_all_preds:
                ...
                if self.args.use_reinforce:
                    ...
                    return output.squeeze(1), r_all, None, None, None #torch.stack(base_out_list, dim=1)
    ...
    def combine_logits(self, r, base_out_list, ind_list):
        ...
        b_, t_, k_ = r.shape
        #when N == 1
        total_cnt = torch.tensor(0.0).cuda()
        total_feat = torch.zeros(1,1,self.num_class).cuda()
        for i in range(len(base_out_list)-1):
            for j in range(len(base_out_list[i])):
                total_cnt +=1
                try:
                    total_feat += base_out_list[i][j]
                except:
                    pdb.set_trace()
        return total_feat/(total_cnt).clamp(min=1)
